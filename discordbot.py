...
# .env 파일에서 PREFIX, TOKEN 정보를 가져옵니다.
load_dotenv()
...
# 명령어
async def handle_command_list(message):
    command_list = [
        '안녕: 봇이 인사를 합니다.',
        '보스: 예상 출현 시간이 가장 빠른 순서로 보스 목록을 출력합니다.',
        '보스이름 컷: 해당 보스의 예상 출현 시간이 컷을 입력한 시간으로 변경합니다.',
        '보스이름 시간: 해당 보스의 예상 출현 시간을 입력한 시간으로 변경합니다.',
        '보스이름 초기화: 해당 보스의 타이머를 초기화합니다.'
    ]
    command_str = "\n".join([f"- {command}" for command in command_list])
    await message.channel.send(f"사용 가능한 명령어 목록:\n{command_str}")


# 각 보스의 타이머 정보를 저장할 딕셔너리를 생성합니다.
boss_timers = {}

# 메시지가 도착하면 실행되는 이벤트 핸들러입니다.
@client.event
async def on_message(message):
    ...
    # "보스이름 초기화" 형태의 메시지에 대한 처리입니다.
    elif message.content.endswith('초기화'):
        # 메시지를 분리하여 "보스이름"을 추출합니다.
        split_msg = message.content.split()
        boss_name = split_msg[0]

        # 보스 이름이 존재하지 않는 경우, 에러 메시지를 전송합니다.
        if boss_name not in boss_list:
            await message.channel.send(f"{boss_name} 보스는 존재하지 않는 보스입니다.")
        else:
            # 해당 보스의 타이머 정보를 삭제합니다.
    ...
    # "보스이름 시간" 형태의 메시지에 대한 처리입니다.
        ...
        if boss_name not in boss_list:
            ...
            # 입력된 시간을 파싱합니다.
            try:
                expected_time_kst = datetime.datetime.strptime(split_msg[1], '%H%M')
            except ValueError:
                await message.channel.send('잘못된 시간 형식입니다. (HHMM)')
                return
            ...
            # 작성자 아이디를 가져와 메시지를 전송합니다.
            author_name = message.author.name
            await message.channel.send(f"{boss_name} 보스 출현 시간이 변경되었습니다. / {author_name}")
